...
install:
	...
	$(shell mv ../stan/src/stan/lang/ast.hpp install_files/ast_vanilla_stan.hpp)
	$(shell [ ! -f install_files/ast_def_vanilla_stan.cpp] mv ../stan/src/stan/lang/ast_def.cpp install_files/ast_def_vanilla_stan.cpp)
	$(shell cp install_files/ast_modified.hpp ../stan/src/stan/lang/ast.hpp && \
	cp install_files/ast_def_modified.cpp ../stan/src/stan/lang/ast_def.cpp)
	# Create a backup of STAN function_signatures.h file
	$(shell [ ! -f install_files/function_signatures_vanilla_stan.h ] mv ../stan/src/stan/lang/function_signatures.h install_files/function_signatures_vanilla_stan.h)
...
reload_pwa_library:
	...
	$(shell [ -f ! install_files/function_signatures_vanilla_stan.hpp ] mv ../stan/src/stan/lang/function_signatures.h install/function_signatures_vanilla_stan.h)
	$(shell cp install/function_signatures_modified.h ../stan/src/stan/lang/function_signatures.h)
	# Tell Stan where the files of stan_pwa are stored
	$(shell [ -f ! install_files/math_vanilla_stan.hpp ] mv ../stan/lib/stan_math_2.7.0/stan/math.hpp install_files/math_vanilla_stan.hpp)
	$(shell cp install_files/math_modified.hpp ../stan/lib/stan_math_2.7.0/stan/math.hpp)
