...
{ 
  ...
  /*
   * double norm_background(vector theta_background_abs2_, 
   *                        vector I_background_abs2_)
   *
   * Takes two real vectors, returns their scalar product.
   * Needed for bookkeeping purposes.
   */
  template <typename T2, typename T3>
  typename boost::math::tools::promote_args<T2,T3>::type
  norm_background(const Eigen::Matrix<T2, Eigen::Dynamic, 1>& theta_background_abs2_,
       const Eigen::Matrix<T3, Eigen::Dynamic, 1>& I_background_abs2_) {

    return mr::vector::mult(theta_background_abs2_,I_background_abs2_);
  }
