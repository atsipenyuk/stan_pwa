...
/***************************************************************************
 *   Following functions are added from stan_pwa                           *
 *                                                                         *
 * Note that these functions rely on the model you are using. For example, *
 * calling 'num_bins_y1' (number of bins in 1st dimension) for an unbinned *
 * model will result in an error.                                          *
 ***************************************************************************/
// Returns vector of complex model-dependent amplitudes
add("amplitude_vector",expr_type(VECTOR_T,1U),VECTOR_T);
// Returns vector of real model-dependent background |amplitudes|^2
add("background_vector",VECTOR_T,VECTOR_T);

// Returns probability of PWA decay for parameter fitting
// (overloaded for different model types)
add("f_fit",DOUBLE_T,DOUBLE_T,expr_type(DOUBLE_T,1U),DOUBLE_T,expr_type(DOUBLE_T,1U),expr_type(VECTOR_T,1U),expr_type(VECTOR_T,1U), VECTOR_T, VECTOR_T);
add("f_fit",DOUBLE_T,expr_type(DOUBLE_T,1U),expr_type(DOUBLE_T,1U),expr_type(VECTOR_T,1U),expr_type(VECTOR_T,1U));

// Returns probability of PWA decay for event generation
add("f_gen",DOUBLE_T,expr_type(VECTOR_T,1U),expr_type(VECTOR_T,1U), VECTOR_T, VECTOR_T);

/* f_genfit 
 *
 * Returns probability of PWA decay for event generation and parameter fitting
 * (for simpler models, there is no need to distinguish between the two)
 * (overloaded for different model types)
 */
// Model-dependent without background
add("f_genfit",DOUBLE_T,expr_type(VECTOR_T,1U),expr_type(VECTOR_T,1U));
// Model-dependent with background
add("f_genfit",DOUBLE_T,expr_type(VECTOR_T,1U),expr_type(VECTOR_T,1U), VECTOR_T, VECTOR_T);

/* Norm
 *
 * Returns normalization factor for parameter fitting
 * (so that f_fit/Norm = 1 if integrated over y=(m2_ab, m2_bc, ...) )
 * (overloaded for different model types)
 */
// Model-dependent without background
add("norm",DOUBLE_T,expr_type(VECTOR_T,1U),expr_type(MATRIX_T,1U));
// Model-dependent with background
add("norm",DOUBLE_T,expr_type(VECTOR_T,1U),expr_type(MATRIX_T,1U), VECTOR_T, VECTOR_T);
// Model-dependent with background, background only
add("norm_background",DOUBLE_T, VECTOR_T, VECTOR_T);

// Binned models
add("norm_bin",DOUBLE_T,expr_type(VECTOR_T,1U),expr_type(VECTOR_T,1U),expr_type(VECTOR_T,1U),VECTOR_T,VECTOR_T,expr_type(MATRIX_T,1U), expr_type(MATRIX_T,1U), expr_type(MATRIX_T,1U),expr_type(MATRIX_T,1U),VECTOR_T, VECTOR_T);

// Keep track of following things...
add("num_background",INT_T); // number of background amplitudes
add("num_bins_y1",INT_T); // number of bins in 1st dimension
add("num_bins_y2",INT_T); // number of bins in 2nd dimension
add("num_non_S_res",INT_T); // number of resonances in P,D,... waves
add("num_resonances",INT_T); // number of coherently summed amplitudes
// number of variables (2 for 3-body-decay, 5 for 4-body-decay, etc.)
add("num_variables",INT_T); 

// Returns 1 if data point is in the phase space, 0 else
add("phase_space_gen",DOUBLE_T,VECTOR_T);
